...
//
// The PUBLIC header (uikit/menu_bar.h) keeps `UIMenuBar` and `UIMenuBarItem`
...
//
// Two separate child arrays on UIMenuBar (`items[]` for UIMenuBarItem leaves and
// `subs[]` for UIMenuBar submenus) keep the free / detach logic obvious — no
...
struct UIMenuBar {
    ...
    int        n_items;
    int        cap_items;
    UIMenuBarItem** items;        // leaf items in display order
    int        n_subs;
    int        cap_subs;
    UIMenuBar**   subs;           // submenus in display order
#ifdef __OBJC__
...
// sync and ui_menu_bar__free() would leak.
