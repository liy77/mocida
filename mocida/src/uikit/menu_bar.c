// menu_bar.c — cross-platform owner of the UIMenu / UIMenuItem tree.
//
...
__attribute__((weak)) void ui_menu__hook_attach_nsmenu(UIMenu* m);
__attribute__((weak)) void ui_menu__hook_attach_nsitem(UIMenuItem* it,
                                                      UIMenu* parent);
__attribute__((weak)) void ui_menu__hook_attach_nssubmenu(UIMenu* parent,
                                                         UIMenu* sub);
__attribute__((weak)) void ui_menu__hook_install_root(UIMenu* root);
__attribute__((weak)) void ui_menu__hook_uninstall_root(void);
...
UIMenu* ui_menu__alloc(const char* title) {
    ...
    if (ui_menu__hook_attach_nsmenu) ui_menu__hook_attach_nsmenu(m);
    return m;
}
...
void ui_menu__free(UIMenu* m) {
    ...
    for (int i = 0; i < m->n_items; i++) {
        ui_menu_item__free(m->items[i]);
    }
    for (int i = 0; i < m->n_subs; i++) {
        ui_menu__free(m->subs[i]);
    }
    free(m->items);
    free(m->subs);
    free(m->title);
...
UIMenuItem* ui_menu_item__alloc_leaf(const char* label,
                                     const char* shortcut,
                                     uint32_t id) {
    UIMenuItem* it = (UIMenuItem*)calloc(1, sizeof(UIMenuItem));
    if (!it) return NULL;
    it->id       = id;
...
static UIMenu* g_root = NULL;

void ui_menu__set_root(UIMenu* root) {
    if (g_root == root) return;
    if (ui_menu__hook_uninstall_root) ui_menu__hook_uninstall_root();
    g_root = root;
    if (ui_menu__hook_install_root) ui_menu__hook_install_root(root);
}
...
void ui_menu_destroy(UIMenu* m) {
    ...
    if (m == g_root) ui_menu__set_root(NULL);
    ui_menu__free(m);
}
...
UIMenuItem* ui_menu_item_create(const char* label,
                                const char* shortcut,
                                uint32_t id) {
    return ui_menu_item__alloc_leaf(label, shortcut, id);
}

UIMenuItem* ui_menu_separator(void) {
    return ui_menu_item__alloc_separator();
}

void ui_menu_item_destroy(UIMenuItem* it) {
    ui_menu_item__free(it);
}
...
void ui_menu_append_item(UIMenu* parent, UIMenuItem* item) {
    ...
    if (ui_menu__hook_attach_nsitem) ui_menu__hook_attach_nsitem(item, parent);
    ui_menu__append_item(parent, item);
}

void ui_menu_append_submenu(UIMenu* parent, UIMenu* sub) {
    if (!parent || !sub) return;
    if (ui_menu__hook_attach_nssubmenu) ui_menu__hook_attach_nssubmenu(parent, sub);
    ui_menu__append_submenu(parent, sub);
}

void ui_menu_append_separator(UIMenu* parent) {
    if (!parent) return;
    UIMenuItem* it = ui_menu_separator();
    if (!it) return;
    // Single-step: build a separator leaf and attach it in one shot.
    // (The previous attempt recursed into ui_menu_item_create here by
    ...
    ui_menu_append_item(parent, it);
}

void ui_menu_bar_set(UIMenu* root) {
    ui_menu__set_root(root);
}

// ── Item-callback forwarding ─────────────────────────────────────────────
// The callback and its user pointer live in the .mm (it owns the
// NSMenuItem → UIMenuItem bridge via associated objects). On non-Apple
// the .mm is absent, so the weak forwarder is a no-op.
__attribute__((weak)) void ui_menu__hook_set_callback(
    ui_menu_item_callback_t cb, void* user);

void ui_menu_bar_set_item_callback(ui_menu_item_callback_t cb, void* user) {
    if (ui_menu__hook_set_callback) ui_menu__hook_set_callback(cb, user);
}
